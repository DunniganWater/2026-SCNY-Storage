...
def main() -> None:
    ...
    # Zone outlines for the map's boundary overlay. Same four zones regardless
    # of method — in the single method the cells cross these lines, which is
    # exactly why it helps to draw them.
    zb = [{"zone": z,
           "area_acres": round(zones_a[z].area * ACRES_PER_M2, 1),
           "rings": rings_latlng(zones_a[z])}
          for z in ZONE_ORDER]
    region_rings = rings_latlng(region_a)
    lines = [
        "// Auto-generated by scripts/build_polygons.py - do not edit by hand.",
        "// SCNY management-zone outlines + the SCNY region boundary, for the map.",
        "// rings are arrays of [lat, lng] pairs (Leaflet convention).",
        "",
        "const ZONE_BOUNDARIES = " + json.dumps(zb) + ";",
        "const REGION_BOUNDARY = " + json.dumps({"rings": region_rings}) + ";",
        "",
    ]
    (JS_DIR / "zone-boundaries.js").write_text("\n".join(lines))
    print(f"zone outlines: {len(zb)} zones + SCNY region -> js/zone-boundaries.js")
